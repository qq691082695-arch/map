...
const fetchList=async()=>{loading.value=true;try{const r=await run(()=>api.getShops(query));if(r){list.value=r.data.list;total.value=r.data.total}}finally{loading.value=false}}
...
const saveBusiness=()=>formRef.value.validate(async valid=>{if(!valid)return;saving.value=true;try{const payload=businessPayload();const r=await run(()=>form.id?api.updateShop({id:form.id,...payload}):api.addShop(payload));if(r){ElMessage.success(r.message);formVisible.value=false;fetchList()}}finally{saving.value=false}})
const toggleBusiness=async row=>{const r=await run(()=>api.updateShopStatus(row.id,row.status==='ENABLED'?'DISABLED':'ENABLED'));if(r){ElMessage.success(r.message);fetchList()}}
const removeBusiness=row=>ElMessageBox.confirm(`确定逻辑删除「${row.name}」吗？`,'提示',{type:'warning'}).then(async()=>{const r=await run(()=>api.deleteShop(row.id));if(r){ElMessage.success(r.message);fetchList()}}).catch(()=>{})
...
const loadResources=async()=>{resourceLoading.value=true;try{const r=await run(()=>api.getShopItems(current.value.id,typeValue(current.value.businessType),{page:1,size:100}));if(r)resources.value=r.data.list}finally{resourceLoading.value=false}}
...
const saveResource=async()=>{if(!resourceForm.name)return ElMessage.warning(`请输入${resourceLabel.value}`);resourceSaving.value=true;try{const type=typeValue(current.value.businessType),payload=resourcePayload();const r=await run(()=>resourceForm.id?api.updateShopItem(current.value.id,type,{id:resourceForm.id,...payload}):api.addShopItem(current.value.id,type,payload));if(r){ElMessage.success(r.message);resourceFormVisible.value=false;loadResources()}}finally{resourceSaving.value=false}}
const toggleResource=async row=>{const r=await run(()=>api.updateShopItemStatus(current.value.id,typeValue(current.value.businessType),row.id,row.status==='ENABLED'?'DISABLED':'ENABLED'));if(r){ElMessage.success(r.message);loadResources()}}
const removeResource=row=>ElMessageBox.confirm(`确定逻辑删除「${row.name}」吗？`,'提示',{type:'warning'}).then(async()=>{const r=await run(()=>api.deleteShopItem(current.value.id,typeValue(current.value.businessType),row.id));if(r){ElMessage.success(r.message);loadResources()}}).catch(()=>{})
onMounted(fetchList)
</script>
