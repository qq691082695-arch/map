...
OUT = Path(r"D:\mapvendor\地图商家预约系统_后端与数据库PRD_v1.7.docx")
...
font(fp.add_run("内部开发基线 · v1.7 · 2026-08-18"),8.5,False,GRAY)
...
table(["文档属性","内容"],[
    ...
    ["版本/状态","v1.7 / 后端零登录，前后端开发职责拆分"],
    ["编制日期","2026-08-18"],
],[1800,7560])
callout("结论","本开发计划只负责 Java 后端、数据库、OpenAPI、测试和部署。微信小程序与管理员后台前端由其他开发人员负责，且管理员后台已有部分 Vue 页面。后端不修改前端页面，不提供任何登录接口，只按现有页面需求冻结契约并交付真实业务 API。")
doc.add_page_break()
...
h("1.2 MVP 范围",2)
bullets(["高校区域、三类服务商及车辆/菜品/房型的后端 CRUD、启停、逻辑删除与图片接口。","小程序地图数据、分类计数和服务商详情查询接口。","管理员后台与小程序登录均由前端完成；后端不实现任何登录，只接收业务请求及前端传入的 openid。","三类预约创建、本人订单查询及待确认取消接口。","管理员订单查询、确认、取消、统计和导出接口。","统一响应、异常、参数校验、日志、数据库迁移与部署配置。","向前端负责人交付 OpenAPI、字段映射、环境地址与联调说明；不实现或修改前端页面。"])
h("1.3 已有前端现状与协作边界",2)
table(["项目","已完成内容","当前状态/后端动作"],[
    ...
    ["前端登录","管理员后台已有固定演示账号与本地 token；微信登录在小程序前端完成","均属于前端实现，后端禁止增加登录、密码或令牌接口"],
],[1500,3860,4000])
...
table(["维度","验收口径"],[
    ...
    ["安全","后端不做任何登录认证；/admin/** 无后端认证的公网暴露风险必须记录，并通过 Nginx 网络边界、HTTPS、限流和审计降低风险；openid 可被伪造。"],
...
table(["角色","身份方式","允许操作","禁止操作"],[
    ["微信用户","前端完成微信登录并向后端传入 openid；后端不参与登录","浏览启用内容；创建、查看、取消该 openid 的待确认预约","取消已确认订单；处理订单"],
...
table(["编号","需求","后端规则/验收"],[
    ...
    ["BOUND-02","小程序登录","完全由前端完成；后端不调用 code2Session、不维护微信密钥、不签发令牌；订单接口接收 openid。"],
...
table(["层","建议"],[
    ...
    ["登录边界","后端不引入登录认证、密码校验、Session、JWT 或令牌；管理员后台与小程序登录均由前端完成"],
...
h("6.2 逻辑架构",2)
bullets(["Controller：协议适配、请求上下文、参数校验，不承载业务事务或登录认证。","Application/Service：用例编排、事务、业务规则与状态机。","Domain：枚举、规则、领域异常、订单状态转换。","Repository/Mapper：数据库访问；避免 Controller 直接调用 Mapper。","Integration：文件存储、Excel 等外部能力，使用接口隔离；不创建任何登录集成模块。","Query：地图聚合、管理员统计与导出使用专用查询。"])
h("6.3 安全与隐私",2)
bullets(["后端不保存或校验任何登录账号、密码、Session、JWT 或令牌。","上传白名单 JPEG/PNG/WebP，校验真实 MIME、大小、随机文件名，阻断路径穿越和脚本文件。","SQL 参数化；CORS 精确配置管理员后台域名。","手机号属于个人信息：最小采集、传输 HTTPS、日志脱敏、导出审计、明确保留期限。","小程序 openid 由前端直接传入存在伪造风险；不得把它描述为强身份认证。","接口限流：管理员写操作、下单和导出重点保护；数据库密码用环境变量/密钥管理；/admin/** 通过 Nginx 限制访问来源。"])
h("6.4 缓存策略",2)
...
h("8.1 最低验收用例",2)
nums(["后端代码中不存在任何登录接口、账号密码校验、Session、JWT、管理端令牌、code2Session、微信密钥、商家账号或 /merchant/**。","/admin/** 不做后端登录认证；Nginx 网络边界、HTTPS、限流与审计配置经过验证。","同一待确认订单被管理员确认与用户取消并发操作时，只有一个成功。","确认后用户取消必须失败；待确认订单允许对应 openid 取消。","禁用服务商不出现在地图/详情，且不能新建预约；历史订单仍可查看。","出行车辆/住宿房间必须属于对应服务商，车辆数量和房间数量为正整数。","管理员取消未填写原因时失败；非待确认状态重复处理失败。","按 service_date 和三种状态统计，管理员统计与订单明细一致。","上传伪装脚本、超大文件、非法 MIME 或路径穿越文件名均被拒绝。"])

h("9. 风险清单与应对")
table(["等级","风险","影响","应对"],[
    ["高","前端直接传入 openid","openid 可被伪造，无法形成后端强认证边界","按用户确认范围实施；全程 HTTPS、日志脱敏、限流并明确记录风险"],
...
h("11. 已确认决策与剩余问题")
table(["状态","事项","结论"],[
    ["已确认","小程序身份","微信登录由前端完成；后端不参与登录，只接收、保存并使用前端传入的 openid"],
